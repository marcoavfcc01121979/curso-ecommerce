...
module.exports = ({ usuario, recovery }, cb) => {
  const message = `
    ...
    <br/>
    <p>
      Aqui esta o link para redefinir a sua senha. Acesse ele e digite sua nova.
    </p>
  ...
  if(process.env.NODE_ENV === "production") {
    transporter.sendMail(opcoesEmail, (error, info) => {
      if(error) {
        ...
        return cb(null, "Link para redfinição de senha foi enviado com sucesso para seu email.");
      }
    });
  }else {
