...
class ClienteController{
  ...
  async showAdmin(req, res, next) {
    try {
      const cliente = await (await Cliente.findOne({ _id: req.params.id, loja: req.query.loja })).populate("usuario");
 ...
 */
 ...
 async show(req, res, next) {
   try{
    const clientes = await Cliente.findOne({ usuario: req.payload.id, loja: req.query.loja }).populate('usuario');
    return res.send({ clientes });
   } catch(e) {
     next(e)
   }
